# python -c "import bitcoinPuzzle_v4; bitcoinPuzzle_v4.measure_performance(19, 20, '1HsMJxNiV7TLxmoF6uJNkydxPFDog4NQum', 12)"
...
def batch_key_generation(start, end, target_address, found_flag, lock):
    ...
    for i in range(start, end):
        ...
        if res:
            with lock:
                if not found_flag.value:  # Double-check if key wasn't already found
                    found_flag.value = True
                    found_key = res
            break
    return found_key

def measure_performance(min_exp, max_exp, target_address, num_workers = 1):
    ...
    with Manager() as manager:
        ...
        with concurrent.futures.ProcessPoolExecutor(max_workers=num_workers) as executor:
            future_to_index = {
                executor.submit(batch_key_generation, start, min(start + batch_size, max_val + 1), target_address, found_flag, lock): start
...
if __name__ == '__main__':
    ...
    num_workers = 12
    measure_performance(20, 21, target_address, num_workers)
